...
- **Crossing motion:** breakpoint crossings with an open detail animate in both directions — into compact the detail grows out of its pane (inline/overlay) or plays the route's real entrance (route mode); into expanded the list slides in beside the full-width detail. Pane geometry keeps tracking window drags without motion.
